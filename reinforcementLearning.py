...
def advanced_agent(obs, config, action):
    ...
    # If there are no ships, use the first shipyard to spawn a ship.
    if len(me.ships) == 0 and len(me.shipyards) > 0:
        if ship_ > 0 and ship_ - 1 < len(me.shipyards):
            me.shipyards[ship_ - 1].next_action = ShipyardAction.SPAWN
        else:
            me.shipyards[0].next_action = ShipyardAction.SPAWN  # Default to the first shipyard

    # If there are no shipyards, convert the first ship into a shipyard.
    if len(me.shipyards) == 0 and len(me.ships) > 0 and ship_ == 0:
        me.ships[0].next_action = ShipAction.CONVERT

    try:
        if act == 'CONVERT' and len(me.ships) > 0:
            me.ships[0].next_action = ShipAction.CONVERT
            update_L1()
            if len(me.ships) == 0 and len(me.shipyards) > 0:
                if ship_ > 0 and ship_ - 1 < len(me.shipyards):
...
while not env.done:
    ...
    with tf.GradientTape() as tape:
        for timestep in range(1, env.configuration.episodeSteps + 200):
            # Convert the state into tensor format
